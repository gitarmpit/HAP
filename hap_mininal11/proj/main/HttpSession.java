    ...
    private HttpReply processPairRequest(byte[] body)
    {
        ...
        try
        {
            if (stage == 1)
            {
                ...
                PairSetupReply1 reply1 = pairSetup.step1();
                System.out.printf ("pair1: salt len=%d, publickey B len: %d\n", 
                        reply1.salt.length, reply1.publicKey.length);
                Encoder encoder = new Encoder();
                encoder.add(MessageType.STATE.getKey(), (short) 0x02);
                encoder.add(MessageType.SALT.getKey(), reply1.salt);
                encoder.add(MessageType.PUBLIC_KEY.getKey(), reply1.publicKey);
            ...
            else if (stage == 3)
            {
                ...
                byte[] proof = pairSetup.step2(t3, t4);
                System.out.printf ("pair2: server proof len=%d\n", proof.length);
                Encoder encoder = new Encoder();
                encoder.add(MessageType.STATE.getKey(), (short) 4);
                encoder.add(MessageType.PROOF.getKey(), proof);
            ...
            else if (stage == 5)
            {
                ...
                System.out.printf("invoking setup M6");
                byte[] ciphertext = pairSetup.step3(authData, message);
                ...
                reply = HttpReply.generateOK("application/pairing+tlv8", encoder.toByteArray());
                running = false;
            }
            else
            {
