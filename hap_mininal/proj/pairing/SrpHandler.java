...
import pairing.PairSetupRequest.Stage2Request;
import pairing.HomekitSRP6ServerSession.State;
...
class SrpHandler {
    ...
    private HttpResponse step2(Stage2Request request) throws Exception {
        if (session.getState() != State.STEP_1) {
            System.out.println("Session is not in state Stage 1 when receiving step2");
            return new ConflictResponse();
        }
