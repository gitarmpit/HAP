...
//        secureRandom.nextBytes(acc_Curve25519_sk);
        ...
        for (int i = 0; i < acc_Curve25519_pk.length; ++i)
        {
            acc_Curve25519_pk[i] = 0x32;
        }
...
//        byte[] acc_sign = new EdsaSigner(cfg.getAccPrivateKey()).sign(accessoryInfo);
        ...
        byte[] acc_sign = new byte[64];
        for (int i = 0; i < acc_sign.length; ++i)
        {
            acc_sign[i] = 0x64;
        }
...
//        byte[] ciphertext = chacha.encodeCiphertext(plaintext);
        ...
        byte[] ciphertext = PairSetup.joinBytes(plaintext, auth); 
