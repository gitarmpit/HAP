...
void HttpSrv::removeClient (SOCKET client_s, int connectionId) 
{
        ...
        _close_socket (client_s);
        procArray[connectionId-1].reset();
        nfds--;
}
...
void HttpSrv::processClient (SOCKET client_s, int connectionId)
{
    ...
    else if (rc != 1) //not would block 
    {
        ...
        const buf& msg_out = procArray[connectionId-1].process (raw_in);
...
int HttpSrv::readFromClient (SOCKET s, buf& msg) 
{
    static char buf[256]; 
    ...
    while (true) 
    {
        ...
        msg.length += len;
        if (len != sizeof(buf)) //TODO really half-assed, hoping that message is never exactly 256 bytes!
        {
