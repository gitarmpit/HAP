...
HttpReply PairSetup::step3(const buf& message, const buf& authData)
{
    ...
    d.decode(plaintext);
    buf ios_pairing_ID;
    buf ios_Ed25519_ltpk;
    buf ios_Ed25519_proof;

    if (!d.getData(ID, ios_pairing_ID) || 
        !d.getData(PUBLIC_KEY, ios_Ed25519_ltpk) || 
        !d.getData(SIGNATURE, ios_Ed25519_proof)) 
    {
        printf ("M6: no ios ID or PK or signature in incoming  M5 message\n");
