...
//#define DEBUG_T(fmt, ...) ::printf((PGM_P)PSTR(fmt), ## __VA_ARGS__)
#define DEBUG_T(fmt, ...)
...
public:
    ...
    err_t close()
    {
        ...
        if(_pcb) {
            ...
            tcp_err(_pcb, NULL);
            tcp_poll(_pcb, NULL, 0);
            DEBUG_T(":tcp_close before: state %d\r\n", _pcb->state); // 4: ESTABLISHED
            err = tcp_close(_pcb);
            DEBUG_T(":tcp_close after: port %d, err %d, state %d\r\n",
            		_pcb->local_port ,(int) err, _pcb->state); // 5: FIN_WAIT_1
            /*
            if(err != ERR_OK) {
                DEBUGV(":tc err %d\r\n", (int) err);
                tcp_abort(_pcb);
            ...
            DEBUGV(":tcp_close call tcp_abandon\n");
            */
            tcp_abandon(_pcb, 0);
            // tcp_abandon: (state!=CLOSED)
            // 1. call tcp_pcb_remove { TCP_RMV, tcp_pcb_purge, set state = CLOSED }
            // 2. free unacked and unsent (what we really want!)
            // 3. send_rst (when arg1=1, do NOT do this here)
            // 4. report ERR_ABRT
            DEBUG_T(":tcp_abandon after: state %d\r\n", _pcb->state); // 0: CLOSED
            _pcb = nullptr;
        }
        return err;
    }
...
protected:
    ...
    {
    	const size_t data_size = ds->available();
        ...
        if (_sync) {
        	write_ok = wait_until_sent();
        } else {
        	if (data_size != _written) {
        		write_ok = wait_until_sent();
        	}
        }
        if (!write_ok){
        	DEBUG_T(":tcp _write_from_source error, port: %d, call close.\n", _pcb->local_port);
        	close();
        }
