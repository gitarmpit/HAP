...
//        byte[] acc_SRP_proof = bigIntegerToUnsignedByteArray(M2);
        ...
        byte[] acc_SRP_proof = new byte[64];
        for (int i = 0; i < acc_SRP_proof.length; ++i)
        {
            acc_SRP_proof[i] = 0x64;
        }
        
        System.out.printf ("pair2 output: acc SRP proof len=%d\n", acc_SRP_proof.length);
        System.out.println("step2 complete");
        
        return PairHelper.pair2_reply(acc_SRP_proof);
...
//        byte[] acc_Ed25519_ltpk = signer.getPublicKey();
        byte[] acc_Ed25519_ltpk = new byte[32];
        for (int i = 0; i < acc_Ed25519_ltpk.length; ++i)
        {
            acc_Ed25519_ltpk[i] = 0x32;
        }
...
//        byte[] acc_proof = signer.sign(accessoryInfo);  //signature
        ...
        byte[] acc_proof = new byte[64];
        for (int i = 0; i < acc_proof.length; ++i)
        {
            acc_proof[i] = 0x64;
        }
